...
面向 DeepSeek Harness 的纯搜索 `dsh-subscription-search@1.1.0`。保留仓库/包身份与 DSH Web 提供方 id `subscription-search`，但不再拥有 OAuth、凭据同步、订阅用量、模型路由、配额 UI 或 API 密钥输入。
...
手工兜底：在目标 profile 的 dependencies 中加入 `"dsh-subscription-search": "github:shaomingbo/dsh-subscription-search#v1.1.0"`，在 `dsh.profile.bundles` 中加入 `dsh-subscription-search`，再于 profile 目录运行 `pnpm install --ignore-scripts`。优先使用具备原子写入和失败回滚的安装器。
