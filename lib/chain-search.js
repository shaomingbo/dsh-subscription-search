...
const USER_AGENT = 'dsh-subscription-search/1.3.0'
...
/**
 * Configuration state of one credential ref, read through `describe` so the
 * value never crosses this check. Returns undefined when the host exposes no
 * `describe` (older seam), leaving availability honestly 'unknown'.
 */
async function credentialConfigured(credentials, ref) {
  try {
    const info = await credentials.describe(ref)
    return info?.configured === true ? 'available' : info?.configured === false ? 'unavailable' : undefined
  } catch {
    return undefined
  }
}
...
export function createExaBackend({ credentials, fetchImpl = fetch }) {
  return {
    id: 'exa',
    label: 'Exa',
    /** Configuration probe for `list()`: 'available' iff EXA_API_KEY resolves somewhere. */
    async status() {
      return { availability: await credentialConfigured(credentials, 'EXA_API_KEY') }
    },
    /** Search-leg gate: skip the network round-trip when no key can resolve. */
    async available() {
      return (await credentialConfigured(credentials, 'EXA_API_KEY')) !== 'unavailable'
    },
...
export function createDeepSeekBackend({ credentials, fetchImpl = fetch }) {
  return {
    id: 'deepseek',
    label: 'DeepSeek',
    /** Configuration probe for `list()`: 'available' iff DEEPSEEK_API_KEY resolves somewhere. */
    async status() {
      return { availability: await credentialConfigured(credentials, 'DEEPSEEK_API_KEY') }
    },
    /** Search-leg gate: skip the network round-trip when no key can resolve. */
    async available() {
      return (await credentialConfigured(credentials, 'DEEPSEEK_API_KEY')) !== 'unavailable'
    },
    async search(request, signal) {
...
function boundSnippet(value) {
  return value.length > MAX_SNIPPET_CHARS ? `${value.slice(0, MAX_SNIPPET_CHARS)}…` : value
}

/**
 * Built-in Ollama adapter over Ollama's hosted web search API; the credential
 * remains behind the ordinary DSH credential ref. Participation is credential-gated:
 * `available()` skips the leg while OLLAMA_API_KEY is unconfigured, so the chain
 * falls through without a network request.
 */
export function createOllamaBackend({ credentials, fetchImpl = fetch }) {
  return {
    id: 'ollama',
    label: 'Ollama',
    /** Configuration probe for `list()`: 'available' iff OLLAMA_API_KEY resolves somewhere. */
    async status() {
      return { availability: await credentialConfigured(credentials, 'OLLAMA_API_KEY') }
    },
    /** Search-leg gate: skip the network round-trip when no key can resolve. */
    async available() {
      return (await credentialConfigured(credentials, 'OLLAMA_API_KEY')) !== 'unavailable'
    },
    async search(request, signal) {
      const apiKey = await credential(credentials, 'OLLAMA_API_KEY', signal)
      const response = await safeFetch(fetchImpl, `${OLLAMA_BASE_URL}/api/web_search`, {
        method: 'POST',
        redirect: 'error',
        headers: {
          authorization: `Bearer ${apiKey}`,
        ...
        body: JSON.stringify({ query: request.query, max_results: OLLAMA_MAX_RESULTS }),
        signal,
      }, 'Ollama')
      if (!response.ok) throw new SearchChainError('SEARCH_BACKEND_HTTP_ERROR', `Ollama search failed (HTTP ${response.status})`)
      // ollama#16045: an exhausted quota answers 200 OK with an empty body. A response
      // without the documented results array is invalid and must fall through, while a
      // well-formed empty array stays a valid empty result.
      const payload = await response.json().catch(() => null)
      if (!isRecord(payload) || !Array.isArray(payload.results)) {
        throw new SearchChainError('SEARCH_BACKEND_INVALID_RESPONSE', 'Ollama returned an invalid response')
      }
      const seen = new Set()
      const sources = []
      for (const entry of payload.results) {
        if (!isRecord(entry) || typeof entry.url !== 'string' || !URL.canParse(entry.url) || seen.has(entry.url)) continue
        seen.add(entry.url)
        const snippet = typeof entry.content === 'string' && entry.content.trim() !== ''
          ? boundSnippet(entry.content)
          : undefined
        sources.push({
          url: entry.url,
          ...typeof entry.title === 'string' && entry.title !== '' ? { title: entry.title } : {},
