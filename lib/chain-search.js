...
const USER_AGENT = 'dsh-subscription-search/1.2.0'
...
/** Settings badges use describe when present so the value never crosses the check. */
async function credentialConfigured(credentials, ref, signal) {
  if (signal?.aborted) throw signal.reason
  if (typeof credentials.describe === 'function') {
    try {
      return (await credentials.describe(ref, signal))?.configured === true
    } catch {
      return false
    }
  }
...
async function credentialStatus(credentials, ref, signal) {
  return { availability: await credentialConfigured(credentials, ref, signal) ? 'available' : 'unavailable' }
}
...
export function createExaBackend({ credentials, fetchImpl = fetch }) {
  return {
    ...
    label: 'Exa',
    async status(signal) {
      return credentialStatus(credentials, 'EXA_API_KEY', signal)
    },
...
export function createDeepSeekBackend({ credentials, fetchImpl = fetch }) {
  return {
    ...
    label: 'DeepSeek',
    async status(signal) {
      return credentialStatus(credentials, 'DEEPSEEK_API_KEY', signal)
    },
    async search(request, signal) {
...
/**
 * Built-in Ollama adapter over the hosted ollama.com web_search API; the credential
 * remains behind the ordinary DSH credential ref. Participation is credential-gated:
 * `available()` re-resolves `OLLAMA_API_KEY` per operation, so an unconfigured key
 * marks the leg unavailable and the chain falls through without a network request.
 */
export function createOllamaBackend({ credentials, fetchImpl = fetch }) {
  const backend = {
    ...
    label: 'Ollama',
    async status(signal) {
      return credentialStatus(credentials, 'OLLAMA_API_KEY', signal)
    },
    async available(signal) {
      return (await backend.status(signal)).availability === 'available'
    },
    async search(request, signal) {
  ...
  backend.refreshAvailability = async () => (await backend.status()).availability
  return backend
}
