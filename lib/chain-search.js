...
export function createOllamaBackend({ credentials, fetchImpl = fetch }) {
  const backend = {
    ...
    async available() {
      try {
        const resolved = await credentials.resolve('OLLAMA_API_KEY')
  ...
  backend.refreshAvailability = async () => {
    backend.status.availability = await backend.available() ? 'available' : 'unavailable'
    return backend.status.availability
  }
  return backend
}
