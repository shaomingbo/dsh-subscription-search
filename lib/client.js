window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function searchRpc(connection, endpoint, payload) {
      const modern = `subscription-search/${endpoint}`
      return connection.rpc.call(API_CHANNEL, modern, payload).catch(() => connection.rpc.call(CHANNEL, endpoint, payload))
    }
    ...
    function Section({ connection, locale }) {
      ...
      React.useEffect(() => {
        let live = true
        void searchRpc(connection, 'status', {}).then(response => {
          if (!live) return
      ...
      return h('div', { style: section },
        ...
        h('div', { style: actionRow },
          h('button', { type: 'button', style: saving ? disabledButton : primaryButton, disabled: saving, onClick: async () => {
            ...
            try {
              const response = await searchRpc(connection, 'update-settings', { settings: draft })
              if (!response.ok) throw new Error(response.error.message)
