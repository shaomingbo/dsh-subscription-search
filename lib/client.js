window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Section({ connection, locale }) {
      ...
      const rows = draft.order.map((id, index) => {
        ...
        return h('tr', { key: id },
          h('td', { style: cell }, h('input', {
            type: 'checkbox', checked: draft.enabled[id] !== false,
            onChange: event => setDraft({ ...draft, enabled: { ...draft.enabled, [id]: event.target.checked } }),
          })),
          h('td', { style: cell }, `${index + 1}. ${entry.label}`),
          h('td', { style: cell }, availability(entry, t)),
          h('td', { style: cell },
            h('button', { type: 'button', disabled: index === 0, onClick: () => updateOrder(index, -1) }, t('up')),
            ' ', h('button', { type: 'button', disabled: index === draft.order.length - 1, onClick: () => updateOrder(index, 1) }, t('down')),
          ),
        )
      })
      return h('div', { style: section },
        ...
        h('table', { style: table }, h('thead', null, h('tr', null,
          h('th', { style: cell }, t('enabled')), h('th', { style: cell }, t('provider')),
          h('th', { style: cell }, t('status')), h('th', { style: cell }),
        )), h('tbody', null, ...rows)),
        h('label', null, t('perLeg'), h('input', { type: 'number', min: 1, max: 300000, value: draft.perLegTimeoutMs, style: numberInput,
          onChange: event => setDraft({ ...draft, perLegTimeoutMs: Number(event.target.value) }) })),
        h('label', null, t('total'), h('input', { type: 'number', min: 1, max: 300000, value: draft.totalTimeoutMs, style: numberInput,
          onChange: event => setDraft({ ...draft, totalTimeoutMs: Number(event.target.value) }) })),
        h('button', { type: 'button', disabled: saving, onClick: async () => {
        ...
        message ? h('p', { style: secondary }, message) : null,
        h('h3', null, t('diagnostics')),
        status.diagnostics.length === 0 ? h('p', { style: secondary }, t('noDiagnostics')) : h('table', { style: table },
          h('thead', null, h('tr', null, h('th', { style: cell }, t('outcome')), h('th', { style: cell }, t('attempts')))),
    ...
    const secondary = { margin: 0, color: 'var(--dsw-text-secondary, #6b7280)', fontSize: 13 }
    const accountNotice = { padding: 10, borderRadius: 6, background: 'var(--dsw-bg-muted, #f3f4f6)' }
    const table = { width: '100%', borderCollapse: 'collapse', fontSize: 13 }
    const cell = { textAlign: 'left', padding: '7px 10px 7px 0', borderBottom: '1px solid var(--dsw-border, #e5e7eb)' }
    const numberInput = { display: 'block', width: 180, marginTop: 4 }
