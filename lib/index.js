...
/**
 * Envelope codes the published DSH `rpcErrorSchema` accepts alongside an
 * empty `details` object. Anything else fails the client-side result union,
 * so every code leaving this module must land in this whitelist. Codes that
 * carry extra `details` requirements (bad-request, credential-rejected, …)
 * stay out until their payloads are built here too.
 */
...
export function envelopeCode(code) {
  ...
  if (ENVELOPE_CODES.has(code)) return code
...
function failure(message, code = 'internal') {
  const safe = envelopeCode(code)
  const tag = typeof code === 'string' && code !== safe ? `[${code}] ` : ''
  return { ok: false, error: { code: safe, message: `${tag}${message}`, details: {} } }
}
